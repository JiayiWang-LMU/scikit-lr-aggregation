...
from .utils._ranking cimport rank_data_view, rank_data_pointer
...
cdef class RankAggregationAlgorithm:
    ...
    def get_algorithm(cls, algorithm, **kwargs):
        ...
        **kwargs : dict
        ...
        if rank_algorithm in {BPALIAMP2Algorithm, Borda, Copeland}:
...
cdef class PairOrderMatrixBuilder:
    ...
    cdef void init(self, INT64_t n_classes):
        ...
        self.precedences_matrix = np.zeros(
            (n_classes, n_classes, 3), dtype=np.float64)
    ...
    cdef void update_precedences_matrix(
        ...
        for f_class in range(n_classes):
            ...
            for s_class in range(n_classes):
                if y[f_class] == RANK_TYPE.RANDOM or y[s_class] == RANK_TYPE.RANDOM:
                    continue
                condition = y[s_class] <= y[f_class]
                precedences_matrix[f_class, s_class, 2] += condition * weight / denominator
...
cdef class Borda(RankAggregationAlgorithm):
    """Borda algorithm."""        
    ...
    cdef void reset(self,
        ...
        self.count_builder.reset()
        self.pair_order_matrix_builder.reset(precedences_matrix)

    cdef void update_params(self,
                            INT64_t_1D y,
                            DTYPE_t weight,
                            DTYPE_t_1D count=None,
                            DTYPE_t_3D precedences_matrix=None) nogil:
        """Update the parameters."""
        self.pair_order_matrix_builder.update_precedences_matrix(y,
                                                                 weight,
                                                                 precedences_matrix)

    cdef void build_params(self,
                           INT64_t_2D Y,
                           DTYPE_t_1D sample_weight,
                           DTYPE_t_1D count=None,
                           DTYPE_t_3D precedences_matrix=None) nogil:
        """Build the parameters."""
        self.pair_order_matrix_builder.build_precedences_matrix(Y,
                                                                sample_weight,
                                                                precedences_matrix)

    cdef void aggregate_params(self,
                               INT64_t_1D consensus,
                               DTYPE_t_1D count=None,
                               DTYPE_t_3D precedences_matrix=None) nogil:
        """Aggregate the parameters."""
        self.pair_order_matrix_builder.build_pair_order_matrix(precedences_matrix)
        self.count_builder.reset()
        ...
        cdef DTYPE_t_2D pair_order_matrix = self.pair_order_matrix_builder.pair_order_matrix
        count = self.count_builder.count
        ...
        for f_class in range(n_classes):
            for s_class in range(n_classes):
                if f_class == s_class:
                    continue
                count[f_class] -= pair_order_matrix[f_class, s_class]
...
cdef class Copeland(RankAggregationAlgorithm):
    """Copeland algorithm."""        
    ...
    cdef void aggregate_params(self,
        ...
        for f_class in range(n_classes):
            for s_class in range(n_classes):
                if f_class == s_class:
                    continue
                if pair_order_matrix[f_class, s_class] > 0.5 + self.beta:
                   count[f_class] -= 1
                elif pair_order_matrix[f_class, s_class] < 0.5 - self.beta:
                    count[f_class] -= 0
                else:
                    count[f_class] -= 0.5
...
cdef inline bint is_close(double a, double b, double tol=1e-8) nogil:
    """
    Check if two floating point numbers are close to each other.
    """
    return fabs(a - b) < tol

cdef inline double round_nogil(double value, int decimals=10) nogil:
    """
    Round a value to a specific decimal.
    """
    cdef double factor = 10 ** decimals
    return round(value * factor) / factor


cdef class MC1(RankAggregationAlgorithm):
    """Markov chain algorithm 1."""

    cdef void init(self, INT64_t n_classes):
        """Initialize the parameters."""
        self.pair_order_matrix_builder.init(n_classes)

        self.eigen_values_real = np.zeros(n_classes, dtype=np.float64)
        self.eigen_values_imag = np.zeros(n_classes, dtype=np.float64)
        self.eigen_vectors_right = np.zeros((n_classes, n_classes), dtype=np.float64)

        self.work = np.zeros(4 * n_classes, dtype=np.float64)

        self.target_eigenvector = np.zeros(n_classes, dtype=np.float64)
        self.stationary_distribution = np.zeros(n_classes, dtype=np.float64)
    ...
    cdef void reset(self,
        ...
        self.eigen_values_real[:] = 0.0
        self.eigen_values_imag[:] = 0.0
        self.eigen_vectors_right[:] = 0.0

        self.work[:] = 0.0

        self.target_eigenvector[:] = 0.0
        self.stationary_distribution[:] = 0.0
    ...
    cdef void aggregate_params(self,
        ...
        """Aggregate the parameters."""
        if precedences_matrix is None:
            precedences_matrix = self.pair_order_matrix_builder.precedences_matrix
        ...
        cdef DTYPE_t s_precedences
        cdef DTYPE_t ties
        cdef DTYPE_t sum_row
        cdef DTYPE_t total_sum

        cdef INT64_t n_classes = consensus.shape[0]

        cdef int n = n_classes
        cdef int lda = n_classes
        cdef int ldvl = n_classes
        cdef int ldvr = n_classes
        cdef int lwork = 4 * n_classes
        cdef int info = 0

        cdef int close_to_1_idx = -1

        # Transform the pair order matrix to a transition matrix
        for f_class in range(n_classes):
            for s_class in range(n_classes):
                s_precedences = precedences_matrix[s_class, f_class, 0]
                ties = precedences_matrix[s_class, f_class, 1]
                pair_order_matrix[f_class, s_class] = s_precedences + ties + EPSILON

        for f_class in range(n_classes):
            sum_row = 0
            for s_class in range(n_classes):
                sum_row += pair_order_matrix[f_class, s_class]
            for s_class in range(n_classes):
                pair_order_matrix[f_class, s_class] /= sum_row

        # Transpose the transition matrix
        cdef DTYPE_t_2D transition_matrix_transpose = pair_order_matrix.T

        # Calculate eigenvalues and eigenvectors
        dgeev("N",
              "V",
              &n,
              &transition_matrix_transpose[0, 0],
              &lda,
              &self.eigen_values_real[0],
              &self.eigen_values_imag[0],
              NULL,
              &ldvl,
              &self.eigen_vectors_right[0, 0],
              &ldvr,
        ...
        # Find eigenvector corresponding to eigenvalue close to 1
        for f_class in range(n_classes):
            if is_close(self.eigen_values_real[f_class], 1.0):
                close_to_1_idx = f_class
                break

        for f_class in range(n_classes):
            self.target_eigenvector[f_class] = self.eigen_vectors_right[close_to_1_idx, f_class]

        # Normalize the eigenvector to get the stationary distribution
        total_sum = 0.0
        for f_class in range(n_classes):
            total_sum += self.target_eigenvector[f_class]

        for f_class in range(n_classes):
            self.stationary_distribution[f_class] = -self.target_eigenvector[f_class] / total_sum

        # Round the distribution and return rankings
        for f_class in range(n_classes):
            self.stationary_distribution[f_class] = round_nogil(self.stationary_distribution[f_class])

        rank_data_view(
            data=self.stationary_distribution,
            y=consensus,
            method=RANK_METHOD.DENSE)


cdef class MC2(RankAggregationAlgorithm):
    """Markov chain algorithm 2."""
    ...
    cdef void aggregate_params(self,
        ...
        # Transform the pair order matrix to a transition matrix
        for f_class in range(n_classes):
            denominator = precedences_matrix[f_class, f_class, 1]
            for s_class in range(n_classes):
                numerator = precedences_matrix[f_class, s_class, 2] + EPSILON
                denominator += EPSILON
                pair_order_matrix[f_class, s_class] = numerator / denominator
...
cdef class MC3(RankAggregationAlgorithm):
    """Markov chain algorithm 3."""
    ...
    cdef void aggregate_params(self,
        ...
        for f_class in range(n_classes):
            ...
            for s_class in range(n_classes):
                if f_class == s_class:
                    continue
                n_samples = precedences_matrix[f_class, f_class, 1] + EPSILON
                pair_order_matrix[f_class, s_class] /= (n_samples * n_classes)
                sum_row += pair_order_matrix[f_class, s_class]
            pair_order_matrix[f_class, f_class] = (1 - sum_row)
...
cdef class MC4(RankAggregationAlgorithm):
    ...
    cdef void aggregate_params(self,
        ...
        for f_class in range(n_classes):
            for s_class in range(n_classes):
                if f_class == s_class:
                    pair_order_matrix[f_class, s_class] = 0
                elif pair_order_matrix[f_class, s_class] > 0.5:
                    pair_order_matrix[f_class, s_class] = 0
                else:
                    pair_order_matrix[f_class, s_class] = 1.0 / n_classes
...
cdef class ML(RankAggregationAlgorithm):
    """Maximum lottery algorithm with HiGHS solver integrated."""
    ...
    cdef void init(self, INT64_t n_classes):
        ...
        self.A = np.zeros((n_classes + 1, n_classes), dtype=np.float64)
        self.A[n_classes, :] = 1.0
        self.x = np.zeros(n_classes, dtype=np.float64)

        # Initialize the HiGHS solver instance with n_classes variables and n_classes + 1 constraints
        self.solver = HighsSolver(n_classes, n_classes + 1)
    ...
    cdef void reset(self,
        ...
        cdef int n_classes = precedences_matrix.shape[0]
    ...
    cdef void aggregate_params(self,
        ...
        cdef DTYPE_t_2D pair_order_matrix_transpose = pair_order_matrix.T

        for f_class in range(n_classes):
            for s_class in range(n_classes):
                self.A[f_class, s_class] = pair_order_matrix[f_class, s_class] - pair_order_matrix_transpose[f_class, s_class]

        # Solve the linear program using the HiGHS solver
        self.solver.solve_lp(self.A, self.x)

        # Round the distribution and return rankings
        for f_class in range(n_classes):
            self.x[f_class] = -round_nogil(self.x[f_class])
